...
aws ec2 create-security-group \
    --group-name roman_numbers_sec_grp-okt\
    --description "This Sec Group is to allow ssh and http from anywhere"
```

- We can check the security Group with these commands
```bash
aws ec2 describe-security-groups --group-names roman_numbers_sec_grp-okt
```
...
```
```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name okt-aws --security-group-ids sg-0367ea2b41ec6d0c6 --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
...
```bash
aws ec2 delete-security-group --group-name roman_numbers_sec_grp-okt
```
